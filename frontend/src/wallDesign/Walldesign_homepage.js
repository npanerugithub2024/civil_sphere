...
const CantileverWallCalculator = () => {
  ...
  const createLabel = (fieldName) => {
    ...
    //   html2canvas(document.querySelector(".wallImageAndDataEntry"), {scale: 0.5}) // Adjust scale as needed
    //     .then(canvas => {
    //       // Initialize jsPDF
    //       const pdf = new jsPDF('p', 'mm', 'a4');
    //       const imgData = canvas.toDataURL('image/jpeg', 0.5); // Use JPEG and adjust quality
    //       const imgWidth = 190; // Adjust width for margins
    //       const pageHeight = pdf.internal.pageSize.height;
    //       const imgHeight = canvas.height * imgWidth / canvas.width;
    //       let heightLeft = imgHeight;
    //       let position = 10; // Start position for the first image
    
    //       pdf.addImage(imgData, 'JPEG', 10, position, imgWidth, imgHeight); // Adjust x,y position for margins
    //       heightLeft -= pageHeight;
    
    //       while (heightLeft >= 0) {
    //         position = heightLeft - imgHeight - 10; // Adjust based on the previous position
    //         pdf.addPage();
    ...
    //         pdf.setPage(i);
    //         pdf.setFontSize(10);
    //         pdf.text('Page ' + String(i) + ' of ' + String(pageCount), pdf.internal.pageSize.width - 30, pdf.internal.pageSize.height - 10);
    //       }
    ...
    function downloadDivAsPDF(selector) {
      ...
      // Initial high DPI setting for quality enhancement
      const dpi = 192;
    
      // Calculate the element's dimensions
      const rect = element.getBoundingClientRect();
      const elementWidthInPx = rect.width;
      const elementHeightInPx = rect.height;
    
      // Calculate the PDF page size based on the element size and desired DPI
      const widthInInches = elementWidthInPx / dpi*2;
      const heightInInches = elementHeightInPx / dpi*2;
    
      // Determine the scale to use for html2canvas. This is crucial for maintaining quality.
      // The scale factor here helps maintain quality when the content is rendered onto the canvas.
      const scale = dpi / 96; // Base DPI for most browsers is 96
      ...
      const options = {
        ...
        html2canvas: {
          ...
          useCORS: true, // Allow images to be loaded from external domains
        },
        jsPDF: {
          orientation: 'portrait',
          unit: 'in',
          format: [widthInInches, heightInInches], // Custom page size based on content size
          compressPDF: true,
        }
      };
      ...
      html2pdf().set(options).from(element).toPdf().get('pdf').then(function (pdf) {
        ...
        for (var i = 1; i <= totalPages; i++) {
          pdf.setPage(i);
          pdf.setFontSize(10);
          pdf.text('Page ' + i + ' of ' + totalPages, 0.5, heightInInches - 0.5); // Adjust as needed
        }
      }).save().catch(err => {
  ...
  return (
    <div className="wall-form-container">
      ...
      <div className = "wallCalculationContents">
        <div className = "CantileverWallButtonRows">
          ...
          <button onClick={toggleToeDesignVisibility} aria-expanded={isToeDesignVisible} className="collapse-button">
            ...
            <button onClick={() => downloadDivAsPDF('.wallImageAndDataEntry')} className="collapse-button">
              DOWNLOAD ALL
            </button>
